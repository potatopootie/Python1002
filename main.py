...
def test():
    ...
    cv_d_c_confirmed = covid_df[["Date","Daily Confirmed", "Cumulative Confirmed", "Still Hospitalised", "In Isolation MOH report", "Total completed Isolation MOH report"]] #use only these columns
    cv_droprows = covid_df.drop(covid_df.index[1:5]) #drop rows or [1,2,3,4]
    print(covid_df.head(10)) #debug printing first 10 lines
    print(cv_d_c_confirmed.head(5))
